...
## 🧱 Core Concepts of TGW

Let's go through every important component:

---
### 1. 🏛️ **Transit Gateway Itself**
...
### 3. 📦 **Transit Gateway Route Table (TGW RT)**
...
✅ **Both must be configured** for traffic to work
...
- Without TGW: Every VPC would need peering with each other → chaos
...
| Issue                                  | Explanation                                           |
| -------------------------------------- | ----------------------------------------------------- |
| You still need VPC route table entries | TGW ≠ magic — VPCs must route traffic through the TGW |
| No transitive DNS                      | TGW doesn't forward DNS between VPCs                  |
| Charges apply per GB                   | You're billed for data going through TGW              |
| CIDRs must not overlap                 | Same as peering — CIDRs should be unique              |